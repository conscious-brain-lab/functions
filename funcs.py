...
def label_diff(ax,means,stds,txt):
	num_bars = len(ax.properties()['xticklabels'][:])
	num_diff = num_bars-1 #factorial(num_bars)/2
	# shell()
	for d in range(num_diff):
		x = range(d,d+2)

		high=np.array(means[d:d+2])+np.array(stds[d:d+2])
		low=np.array(means[d:d+2])-np.array(stds[d:d+2])		
